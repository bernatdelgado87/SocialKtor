...
class NewChallengeUseCase(
    private val repository: ChallengeRepository = ChallengeRepositoryImpl(),
    backgroundCoroutine: BackgroundCoroutine = JobCoroutine()
) : UseCase<ChallengeModel, NewChallengeUseCase.Input>(backgroundCoroutine) {
    data class Input(val courseModel: ChallengeModel, val fileItems: List<PartData.FileItem>)
    ...
    override suspend fun run(params: Input): Flow<ChallengeModel> {
        ...
        return repository.createNewChallenge(params.courseModel)
    }
}
