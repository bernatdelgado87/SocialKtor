...
class NewExerciseUseCase(
    private val repository: ExerciseRepository = ExerciseRepositoryImpl(),
    backgroundCoroutine: BackgroundCoroutine = JobCoroutine()
) : UseCase<ExerciseModel, NewExerciseUseCase.Input>(backgroundCoroutine) {
    data class Input(val courseModel: ExerciseModel, val fileItems: List<PartData.FileItem>)
    ...
    override suspend fun run(params: Input): Flow<ExerciseModel> {
        ...
        return repository.createNewExercise(params.courseModel)
    }
}
