...
class GetAllPostsFromUserUseCase(
...
): UseCase<MultimediaFeed, GetAllPostsFromUserUseCase.Input>(backgroundCoroutine) {
    data class Input(val userId: String)

    override suspend fun run(params: Input): Flow<MultimediaFeed> {
        return repository.getAllPostsFromUser(params.userId.toInt())
    }
